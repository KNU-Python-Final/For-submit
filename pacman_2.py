#23.11.10
#핑크 유령 계속 오렌지랑 겹쳐서 움직여서 시작좌표 바꿈
#141쪽 줄 14~16이 나은듯
#332줄 오타 여러개 -> 미로 밖으로 나가면 유령들 사라짐 -> 수정
#23.11.11
#1020줄쪽에도 핑크 유령 시작좌표 여러개 바꿔야함(다른 애랑 같이 움직임) -> gameover하고도 좌표 초기화하는 거 바꾸기
#903줄 else문 게임 끝나지 않았을 때 moving = True로 하는 걸로 바꾸기 -> 안 그러면 게임 끝나고도 계속 게임 돌아감
#유령 죽고 다시 상자 들어갔다가 살아서 돌아오면 파워업 먹어도 내가 죽음 -> 아직 안 고침 -> 구글 팩맨도 그러는 거 보니 그냥 그게 정상인듯
#23.11.12
#menu화면 다른거 안보고 그냥 만듦
#이겼을 경우 코드 다 바꿔야함 -> 아직 안 바꿈
#gameover랑 win 때 출력하는 애들 while문 안에 넣어주기 -> 안그럼 계속 빤짝거림 -> while문에서 위치 조심!(유령 그리는 거랑 draw_misc()사이여야 안 깜빡임. + 깜빡이게 하고 싶은 거는 draw_misc()뒤에 넣기)
#920줄에 get ready 왜 깜빡임 gameover도 반짝이네??? -> 원인 아직 모름
#유령 모양 바꿈. 하트 모양
#23.11.13
#죽는 거랑 이기는 거랑 동시에 하면 gameover랑 win 동시에 뜸 -> 수정해야함 -> 함~~
#팩맨 여러마리 죽이면 시간 딜레이 돼서 한마리가 상자 밖에서 살아남 -> 수정해야함 -> 아직 안함 ->함(11.18)
#23.11.14
#3라운드 여러 스타일 팩맨 직접 그림 -> 더 추가해야함
#23.11.15
#유령 죽였을 때 점수 100점으로 변경
#pause 추가함
#소리 bgm이랑 파워업bgm 추가함 -> 죽을때,이겼을 때, 졌을 때 추가함
#win 후에 움직이는 오류 발견 -> 수정
#2라운드와 1라운드 연결함.
#23.11.17
#option의 미로색이랑 연결
#23.11.18
...
def pacman():
    ...
    player_x = 450 #플레이어 시작 위치 정함
    player_y = 663
    direction = 0
    #유령 x y좌표, 방향
    blinky_x = 56 #얘 혼자서만 박스 밖에서 시작
    blinky_y = 58
    blinky_direction = 0
    inky_x = 450
    inky_y = 388
    ...
    startup_counter = 0
    lives = 3 #남은 생명 -> 근데 목숨 하나여야하는 거 아닌가 3라운드인데..??
    ...
    class Ghost:
        ...
        def draw(self):
            if (not powerup and not self.dead) or (eaten_ghost[self.id] and powerup and not self.dead): #보통의 상황 -> 걍 파워업 아닌데 생존한 유령, 파워업에 살아있는 유령
                ...
                screen.blit(spooked_img, (self.x_pos, self.y_pos)) #위치에 아이템 사용시 유령 이미지
            else:
            ...
            ghost_rect = pygame.rect.Rect((self.center_x - 18, self.center_y - 18), (36, 36)) #유령 주위에 히트박스..투명한 상자 있다고 생각하고 ->상자 좌표, 크기
            return ghost_rect

        def check_collisions(self): #충돌 확인.
            # R, L, U, D
            num1 = ((HEIGHT - 50) // 32) #높이 한칸
            num2 = (WIDTH // 30)#가로 한칸
            num3 = 15 #오차 -> 벽 한 칸 정중앙에 있음. 30픽셀 -> 15나옴
            self.turns = [False, False, False, False]
            #유령 이동 가능한지 확인
            if 0 < self.center_x // 30 < 29: #밖 안 나가고 미로 안에는 있나
                if level[(self.center_y - num3) // num1][self.center_x // num2] == 9: #위에 유령문이면
                ...
                if self.direction == 2 or self.direction == 3:#위아래
                    if 14 <= self.center_x % num2 <= 16:
                        if level[(self.center_y + num3) // num1][self.center_x // num2] < 3 \
                                or (level[(self.center_y + num3) // num1][self.center_x // num2] == 9 and (
                                self.in_box or self.dead)):
                ...
                if self.direction == 0 or self.direction == 1:#오왼
                    if 14 <= self.center_x % num2 <= 16:
                        if level[(self.center_y + num3) // num1][self.center_x // num2] < 3 \
                                or (level[(self.center_y + num3) // num1][self.center_x // num2] == 9 and (
                                self.in_box or self.dead)):
                ...
                self.turns[0] = True
                self.turns[1] = True #오,왼 못 가니까
            if 390 < self.x_pos < 480 and 370 < self.y_pos < 480: #x,y좌표가 상자 안이면 -> 대충 오차범위 바꿈..
        ...
        def move_clyde(self):
            ...
            if self.direction == 0: #오?
                if self.target[0] > self.x_pos and self.turns[0]: #타겟은 (x,y)쌍 -> 타겟의 x좌표 -> 오른쪽 가는거 현재 가능하면 ㄱㄱ
                    ...
                    if self.target[1] > self.y_pos and self.turns[3]: #아래로 내려가는 거 가능? 타겟 y좌표 더 큼?
                    ...
                    elif self.target[0] < self.x_pos and self.turns[1]: #x좌표 작, 왼쪽 갈 수 있
                        ...
                        self.direction = 1
                        self.x_pos -= self.speed
                elif self.turns[0]: #오른쪽 가는 거 가능. 장애물 없
                ...
                if self.target[0] < self.x_pos and self.turns[1]: #왼쪽이 가능?
                    ...
                    if self.target[0] > self.x_pos and self.turns[0]:
                        ...
                        self.y_pos -= self.speed #걍 디폴트 이동..? 방향이라고해야하나
                ...
                if self.target[1] > self.y_pos and self.turns[3]:#밑이 가능?
                    ...
                    if self.target[0] > self.x_pos and self.turns[0]:
                        ...
                        self.y_pos += self.speed
            if self.x_pos < -30: #왼쪽으로 화면 밖 나가면
                self.x_pos = 900
            elif self.x_pos > 900: #오른쪽으로 화면 밖 나가면
                self.x_pos = -30 #이거 원본은 오타 나있음.
            return self.x_pos, self.y_pos, self.direction


        #밑에 3개 함수 위 함수랑 거의 같은 내용 -> 4마리 살짝씩 다른 행동
    ...
    def draw_pause(): #pause 눌렀을 때
        ...
        screen.blit(press_enter_to_continue_text, (200, 550))
        pygame.display.flip()
    ...
    def draw_misc(): #score 출력
        if not pause:
            ...
            screen.blit(score_text, (590, 920))
            for i in range(lives):
                screen.blit(pygame.transform.scale(heart_img, (30, 30)), (720 + i * 40, 920)) #사이즈 30인 생명마다 40픽셀씩 뛰고 생명 개수 알려줌.
            for j in range(lives,3):
                screen.blit(pygame.transform.scale(empty_heart_img,(30, 30)), (720 + j * 40, 920)) #이미 쓴 빈 하트

            exitbutton = mainmenu.Button(exit_img, click_exit_img, 850, 920, 40, 40, None, action = mainmenu.quitgame)


            if powerup: #파워업 먹었으면
                pygame.draw.circle(screen, 'blue', (138, 932), 10)
    ...
    def check_collisions(scor, power, power_count, eaten_ghosts):
        ...
        num2 = WIDTH // 30
        if 0 < player_x < 870:
            if level[center_y // num1][center_x // num2] == 1: #내 위치 미로 리스트에서 값 찾고 -> 내가 있는 위치 코인 있는 칸일때
                level[center_y // num1][center_x // num2] = 0 #동전 먹었으니까 빈 통로로 바꿈
                scor += 10

            if level[center_y // num1][center_x // num2] == 2: #내 위치 미로 리스트에서 값 찾고 -> 내가 있는 위치 큰코인 있는 칸일때
                level[center_y // num1][center_x // num2] = 0 #큰 코인 먹었으니까 빈 통로로 바꿈
                scor += 50 #큰 코인 하나 50점
    ...
    def draw_board(): #미로 그리기
        num1 = ((HEIGHT - 50) // 32) #맨 밑에 50픽셀 남기고 boards리스트 줄 32줄이니까 그거로 나눔
        num2 = (WIDTH // 30) #가로는 걍 미로 개수로 나눔
        for i in range(len(level)): #행
            for j in range(len(level[i])): #열
                if level[i][j] == 1:
                    pygame.draw.circle(screen, 'white', (j * num2 + (0.5 * num2), i * num1 + (0.5 * num1)), 4) #스크린에 그릴거임,하얀색으로, 그 칸에 정중앙에 할거임,반지름 4
                if level[i][j] == 2 and not flicker:
                    pygame.draw.circle(screen, 'white', (j * num2 + (0.5 * num2), i * num1 + (0.5 * num1)), 10) #반지름 10
                if level[i][j] == 3:
                    pygame.draw.line(screen, color, (j * num2 + (0.5 * num2), i * num1),
                                     (j * num2 + (0.5 * num2), i * num1 + num1), 3) #선 시작, 끝, 선두께 => 얜 선 그리는 거라 y가 그 칸의 맨 위에서 시작임.
                if level[i][j] == 4:
                    pygame.draw.line(screen, color, (j * num2, i * num1 + (0.5 * num1)),
                                     (j * num2 + num2, i * num1 + (0.5 * num1)), 3) #걍 3 가로버전
                if level[i][j] == 5:
                ...
                if level[i][j] == 8:
                    pygame.draw.arc(screen, color,
                                    ...
                                    2 * PI, 3)
                    # 부드럽게 선 이어지게 살짝씩 숫자 조정함
                if level[i][j] == 9:
                    pygame.draw.line(screen, 'white', (j * num2, i * num1 + (0.5 * num1)),
                                     (j * num2 + num2, i * num1 + (0.5 * num1)), 3) #이거 걍 4에서 색만 하얀색으로 바꾼거임
    ...
    def check_position(centerx, centery): #내가 어디있나~
        ...
        if centerx // 30 < 29:
            if direction == 0:#오른쪽 갈때
                if level[centery // num1][(centerx - num3) // num2] < 3: #미로에서 전 칸 찾고->그 칸이 0~2일 때-> 빈 통로, 코인, 큰 코인
                    turns[1] = True #전칸 통로에 포함되면 돌아갈 수 있게
            if direction == 1:#왼쪽 갈때
                if level[centery // num1][(centerx + num3) // num2] < 3: #미로에서 전 칸 찾고->그 칸이 0~2일 때-> 빈 통로, 코인, 큰 코인
                    turns[0] = True
            if direction == 2:#위로 갈때
                if level[(centery + num3) // num1][centerx // num2] < 3: #미로에서 전 칸 찾고->그 칸이 0~2일 때-> 빈 통로, 코인, 큰 코인
                    turns[3] = True
            if direction == 3:
                if level[(centery - num3) // num1][centerx // num2] < 3: #미로에서 전 칸 찾고->그 칸이 0~2일 때-> 빈 통로, 코인, 큰 코인
                    turns[2] = True
            ...
            if direction == 2 or direction == 3: #위로 가든 아래로 가든
                if 11 <= centerx % num2 <= 19: #(12,18 보다 11,19가 더 잘 되는 듯)내가 그 칸의 대충 중간에 있느냐~  -> 칸의 중간 아니면 벽 뚫고 갈수도 있 -> 완벽한 숫자는 아니지만 걍 자연스럽. 6픽셀 크지 않음. 생각보다 -> 900픽셀 30개로 나누면 30이니까 한칸이 30픽셀
                    if level[(centery + num3) // num1][centerx // num2] < 3: #내 바로 밑의 애가 가능하면
                        turns[3] = True
                    if level[(centery - num3) // num1][centerx // num2] < 3: #내 위가 가능하면
                        turns[2] = True #위를 True -> 위로 돌아갈 수 있
                if 11 <= centery % num1 <= 19: #내가 지금 y좌표 한칸의 대충 중간이면 -> 얘도 마참가지로 칸의 중간아니면 벽 뚫고 갈 수 있
                    if level[centery // num1][(centerx - num2) // num2] < 3: #내가 지금 왼쪽 비어있으면
                        turns[1] = True #왼쪽 돌아갈 수 있
                    if level[centery // num1][(centerx + num2) // num2] < 3: #오른쪽 비어있으면
                        turns[0] = True #오른쪽 돌아갈수 있
            if direction == 0 or direction == 1:
                if 11 <= centerx % num2 <= 19: #내가 그 칸의 대충 중간에 있느냐~ 완벽한 숫자는 아니지만 걍 자연스럽. 6픽셀 크지 않음. 생각보다 -> 900픽셀 30개로 나누면 30이니까 한칸이 30픽셀
                    if level[(centery + num1) // num1][centerx // num2] < 3: #내 바로 밑의 애가 가능하면
                        turns[3] = True #밑을 True로 -> 갈 수 있
                    if level[(centery - num1) // num1][centerx // num2] < 3: #내 위가 가능하면
                        turns[2] = True #위를 True -> 갈 수 있
                if 11 <= centery % num1 <= 19: #내가 지금 y좌표 한칸의 대충 중간이면
                    if level[centery // num1][(centerx - num3) // num2] < 3: #내가 지금 왼쪽 비어있으면
                        turns[1] = True #왼쪽 갈 수 있
                    if level[centery // num1][(centerx + num3) // num2] < 3:#오른쪽 비어있으면
                        turns[0] = True#오른쪽 갈수 있
    ...
    def get_targets(blink_x, blink_y, ink_x, ink_y, pink_x, pink_y, clyd_x, clyd_y):
        if player_x < 450: #플레이어 위치
            runaway_x = 900 #플레이어에게서 어떻게 벗어나는지(아이템 먹은 경우) 방향
        else:
            runaway_x = 0
        if player_y < 450:
            runaway_y = 900
        ...
        if powerup: #파워업 먹었고
            if not blinky.dead and not eaten_ghost[0]: #유령 안 죽고 안 먹혔고
                ...
                if 340 < blink_x < 560 and 340 < blink_y < 500: #상자
                    blink_target = (400, 100) #상자에서 나오기
                else:
            ...
            if not inky.dead and not eaten_ghost[1]:#유령 안 죽고 안 먹혔고
                ...
                if 340 < ink_x < 560 and 340 < ink_y < 500: #상자
                    ink_target = (400, 100) #문을 향하는게 목표
                else:
    ...
    while run: #무한루프
        ...
        if startup_counter < 180 and not game_over and not game_won and not pause: #게임 시작하기 전에 3초 시간 주고 시작, 게임 안끝났고 안 이겼으면
            ...
            screen.blit(ready_text, (360, 500))  # 아니 얘 왜 빤짝이지????
            startup_counter += 1
            pygame.display.flip()
        ...
        for i in range(len(level)):
            if 1 in level[i] or 2 in level[i]: #칸 중에 코인이 남아있는 칸 있는가?
                game_won = False #코인 남아있으면 게임 이기는 거 ㄴㄴ
        ...
        if game_over:  # 졌을 경우
            if game_won == False:
                screen.blit(game_over_img, (WIDTH // 2 - 250, 350))
        if game_won:  # 이겼을 경우
            bgm.stop()
            powerup_sound.stop()
            ...
            screen.blit(trophy, (WIDTH // 2 - 400, 300))
            screen.blit(win, (WIDTH // 2 - 250, 370))
        ...
        if game_over:
            if not game_won:
                press_enter_to_restart_text = font.render(f'press [SPACE] to restart', True,
                                                          'white')  # antialias : True -> 선 부드럽게..
                screen.blit(press_enter_to_restart_text, (230, 480))
        if game_won:  # 이겼을 경우
            press_enter_to_continue_text = font.render(f'press [SPACE] to continue', True,
                                                       'white')  # antialias : True -> 선 부드럽게..
            screen.blit(press_enter_to_continue_text, (180, 500))
        ...
        if player_x > 900: #화면 너비 오른쪽으로 넘으면
            player_x = -47
        elif player_x < -50: #왼쪽으로 너무 가서 완전히 사라지는
            player_x = 897

def pacman():
    ...
    player_x = 450 # 플레이어 시작 위치
    player_y = 663
    direction = 0
    # 유령 x좌표, y좌표, 방향
    blinky_x = 56 # 혼자서만 박스 밖에서 시작
    blinky_y = 58
    blinky_direction = 0
    inky_x = 450
    inky_y = 388
    ...
    class Ghost:
        ...
        def check_collisions(self):
            ...
            # 유령 이동 가능한지 확인
            if 0 < self.center_x // 30 < 29: # 밖 안 나가고 미로 안에 있다면
                if level[(self.center_y - num3) // num1][self.center_x // num2] == 9: # 맵에서 9번에 있을 때
            ...
            if 390 < self.x_pos < 480 and 370 < self.y_pos < 480: # x,y좌표가 상자 안이면 
        ...
        def move_clyde(self):
            ...
            if self.direction == 0: 
                if self.target[0] > self.x_pos and self.turns[0]: # 타겟은 (x,y)쌍 -> 타겟의 x좌표 -> 오른쪽 가는거 현재 가능하면 ㄱㄱ
                    ...
                    elif self.target[0] < self.x_pos and self.turns[1]: # x좌표 작음, 왼쪽 갈 수 있음
            ...
            for j in range(len(level[i])): #열
                ...
                if level[i][j] == 4:
                    pygame.draw.line(screen, color, (j * num2, i * num1 + (0.5 * num1)),
                                     (j * num2 + num2, i * num1 + (0.5 * num1)), 3) # 3번 가로버전
                if level[i][j] == 5:
    ...
    while run: # 무한루프
        ...
        if startup_counter < 180 and not game_over and not game_won and not pause: # 게임 시작하기 전에 3초 시간 주고 시작, 게임 안끝났고 안 이겼으면
            ...
            screen.blit(ready_text, (360, 500))  # ???
            startup_counter += 1
            pygame.display.flip()
